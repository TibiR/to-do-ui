 ...
 */
export class ToDoTableDataSource extends DataSource<ToDoItem> {
  data: ToDoItem[];

  constructor(private paginator: MatPaginator, private sort: MatSort, private todoService: ToDoService) {
    super();
    this.todoService.getAll().subscribe((res: ToDoItem[]) => {
      this.data = res.map((e)=>{
        e.updatable = true;
        return e;
      })
